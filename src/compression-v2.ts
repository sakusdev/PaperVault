...
export async function decompressByMethod(data:Uint8Array,method:CompressionMethod):Promise<Uint8Array<ArrayBuffer>>{if(method===0)return new Uint8Array(ownedArrayBuffer(data));if(typeof DecompressionStream==="undefined")throw new Error("このブラウザは圧縮データの復元に対応していません");if(method===1)return streamDecompress(data,"gzip");if(method===2)return streamDecompress(data,"deflate");if(method===3)return rleDecode(await streamDecompress(data,"gzip"));if(method===4)return rleDecode(await streamDecompress(data,"deflate"));throw new Error("未知の圧縮方式です")}
